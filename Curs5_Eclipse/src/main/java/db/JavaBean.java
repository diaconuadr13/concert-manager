...
public class JavaBean {
	...
	public void connect() throws ClassNotFoundException, SQLException, Exception {
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mihnea?useSSL=false", "root", "Dynamita1978");
	...
	public void connect(String bd) throws ClassNotFoundException, SQLException, Exception {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + bd, "root", "Dynamita1978");
		} catch (ClassNotFoundException cnfe) {
			error = "ClassNotFoundException: Nu s-a gasit driverul bazei de date.";
			throw new ClassNotFoundException(error);
		} catch (SQLException cnfe) {
	...
	public void connect(String bd, String ip) throws ClassNotFoundException, SQLException, Exception {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			con = DriverManager.getConnection("jdbc:mysql://" + ip + ":3306/" + bd, "root", "Dynamita1978");
		} catch (ClassNotFoundException cnfe) {
			error = "ClassNotFoundException: Nu s-a gasit driverul bazei de date.";
			throw new ClassNotFoundException(error);
		} catch (SQLException cnfe) {
	...
	public void adaugaMuzician(String nume, String prenume, String datanasterii, String taraorigine, String instrumentprincipal)
		...
		if (con != null) {
			try {
	            ...
	            stmt.executeUpdate("INSERT INTO muzicieni (nume, prenume, datanasterii, taraorigine, instrumentprincipal) VALUES ('" + nume + "', '" + prenume + "', '" + datanasterii + "', '" + taraorigine + "', '" + instrumentprincipal + "');");
	...
	public void adaugaStil(String numestil, String originestil, String perioadapopularitate)
			throws SQLException, Exception {
		if (con != null) {
			try {
				// create a prepared SQL statement
				Statement stmt;
				stmt = con.createStatement();
	            stmt.executeUpdate("INSERT INTO stiluri_muzicale (numestil, originestil, perioadapopularitate) VALUES ('" + numestil + "', '" + originestil + "', '" + perioadapopularitate + "');");
	...
	public void adaugaConcert(long idmuzician, long idstil, String dataconcert, String locatie, String altedetalii)
			throws SQLException, Exception {
		if (con != null) {
			try {
				// create a prepared SQL statement
				Statement stmt;
				stmt = con.createStatement();
	            stmt.executeUpdate("INSERT INTO concerte (idmuzician, idstil, dataconcert, locatie, altedetalii) VALUES ('" + idmuzician + "', '" + idstil + "', '" + dataconcert + "', '" + locatie + "', '" + altedetalii + "');");
	...
	public ResultSet intoarceConcertId(int ID) throws SQLException, Exception {
		ResultSet rs = null;
		try {
			// Execute query
	        String queryString = "SELECT c.idconcert, c.dataconcert, c.locatie, c.altedetalii, m.idmuzician, m.nume, m.prenume, m.datanasterii, m.taraorigine, m.instrumentprincipal, sm.idstil, sm.numestil, sm.originestil, sm.perioadapopularitate FROM concerte c, muzicieni m, stiluri_muzicale sm WHERE c.idmuzician = m.idmuzician AND c.idstil = sm.idstil AND c.idconcert = '" + ID + "';";
			Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
			rs = stmt.executeQuery(queryString); //sql exception
		} catch (SQLException sqle) {
			error = "SQLException: Interogarea nu a fost posibila.";
